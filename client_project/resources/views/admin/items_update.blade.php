...
<body class="bg-slate-100 dark:bg-[#101214]">
        ...
        <div class="px-1 bg-slate-300 mx-auto w-fit py-1 h-fit mt-4 rounded-lg shadow-lg mb-10 dark:bg-[#161A1D]">
            <div class="bg-white w-[500px] border-[1.5px] border-slate-300 rounded-2xl dark:bg-[#22272B] dark:border-[#161A1D]">
                <div class="px-6 py-4 border-b-[1.5px] border-slate-300 flex justify-between items-center dark:border-[#38414A]">
                    <div>
                        <p class="text-xl font-semibold text-slate-800 dark:text-white">items Project Form</p>
                        <p class="text-sm text-slate-500 font-medium dark:text-gray-400">Update items detail</p>
                    </div>
                    <div class="">
                ...
                <div class="px-6 py-4">
                    <form action="{{route('items.update.store', $items->items_id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div>
                            <p class="text-slate-700 font-medium dark:text-gray-200">Items Name </p>
                            <input type="text" name="items_name" value="{{$items->items_name}}" placeholder="Project name..." class="border-[1.5px] border-slate-300 w-full py-1 px-2 mt-2 rounded-md outline-none text-slate-700 focus:ring-1 focus:border-blue-100 transition-all duration-150 dark:bg-[#22272B] dark:border-[#38414A] dark:text-gray-300 dark:focus:border-blue-300" required>
                        </div>
                        <div class="flex">
                            <div class="mt-3 w-1/2 mr-3">
                                <p class="text-slate-700 font-medium dark:text-gray-200">Category</p>
                                <select name="category" id="category"  class="border-[1.5px] mt-2 py-1 px-2 rounded-md text-slate-700 border-slate-300 w-full outline-none focus:ring-1 focus:border-blue-100 transition-all duration-150 dark:bg-[#22272B] dark:border-[#38414A] dark:text-gray-300 dark:focus:border-blue-300">
                                    <option value="" disabled selected>Old: {{$items->category}}</option>
                                    <option value="In progress">In progress</option>
                                    <option value="Pending">Pending</option>
                                    <option value="Complete">Complete</option>
                                    <option value="Due contract">Due contract</option>
                                </select>                                
                            </div>
                            <div class="mt-3 w-1/2">
                                <p class="text-slate-700 font-medium dark:text-gray-200">Progress</p>
                                <input type="number" id="progres" value="{{$items->progres}}" placeholder="1-100..." name="progres" class="border-[1.5px] border-slate-300 w-full py-1 px-2 mt-2 rounded-md outline-none text-slate-700 bg-gray-300 cursor-not-allowed truncate focus:ring-1 focus:border-blue-100 transition-all duration-150 dark:bg-[#22272B] dark:border-[#38414A] dark:text-gray-300 dark:focus:border-blue-300" min="0" max="100" required disabled>  
                            </div>
                        </div>
                        <div class="mt-3 w-full">
                            <p class="text-slate-700 font-medium mb-3 dark:text-gray-200">Stage </p>
                            <textarea name="stage" cols="30" rows="10" class="border-[1.5px] w-full px-2 py-2 border-slate-300 rounded-md outline-none text-slate-700 h-16 focus:ring-1 focus:border-blue-100 transition-all duration-150 dark:bg-[#22272B] dark:border-[#38414A] dark:text-gray-300 dark:focus:border-blue-300" placeholder="Insert Stage here. example : make a texture 3d..."></textarea>
                        </div>
                        <p class="mt-4 mb-3 text-slate-700 font-medium dark:text-gray-200">File 3D</p>
    ...
    <script>
        document.getElementById('angka').addEventListener('input', function (e) {
    ...
    <script>
        ...
        category.addEventListener('change', function() {
            if (this.value === 'In progress') {
                progres.disabled = false;
                progres.classList.remove('bg-gray-300', 'cursor-not-allowed');
            } else if (this.value === 'Complete') {
                progres.value = 100;
                progres.disabled = true;
                progres.classList.add('bg-gray-300', 'cursor-not-allowed');
            } else {
                progres.disabled = true;
                progres.classList.add('bg-gray-300', 'cursor-not-allowed');
            }
